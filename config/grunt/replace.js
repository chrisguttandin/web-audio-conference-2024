...
module.exports = (grunt) => {
    return {
        'csp-production': {
            files: {
                './': ['build/web-audio-conference-2024/browser/**/index.html']
            },
            options: {
                patterns: [
                    {
        ...
        'manifest': {
            files: {
                './': ['build/web-audio-conference-2024/browser/ngsw.json']
            },
            options: {
                patterns: [
                    {
                        ...
                        replacement: (_, filename, extension) =>
                            grunt.file.expand({ cwd: 'build/web-audio-conference-2024', ext: extension }, `assets/${filename}.*`)[0]
                    },
                    {
                        // Replace the hash value inside of the hashTable for "/(index|start).html" because it was modified before.
                        match: /"\/web-audio-conference-2024\/(?<filename>index|start)\.html":\s*"[\da-z]+"/g,
                        replacement: (_, filename) => {
                            return `"/web-audio-conference-2024/browser/${filename}.html": "${computeHashOfFile(
                                `build/web-audio-conference-2024/browser/${filename}.html`,
                                'sha1',
                                'hex'
                            )}"`;
                        }
